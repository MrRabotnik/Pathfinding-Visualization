export default class JPS {
    ...
    jump(point, direction, width) {
        const [x, y] = point.split(",").map(Number);
        const [dx, dy] = direction.split(",").map(Number);
        let nextX = x + dx;
        let nextY = y + dy;

        if (!this.adjacencyList[nextX] || !this.adjacencyList[nextX][nextY]) {
            return null;
        }
        return [nextX, nextY];
    }
    ...
    identifySuccessors(node, width) {
        ...
        for (let direction of currentNeighbors) {
            let jumpPoint = this.jump(node, direction, width);
            if (jumpPoint) {
